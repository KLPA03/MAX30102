# ESP32-S3 MAX30102 Logger (USB Mass Storage)

ESP-IDF v5.x project for ESP32-S3 that:

- Reads **MAX30102 raw RED/IR** samples at **100 Hz** over I2C
- Downsamples to **20 Hz** (averages 5 samples)
- Writes `log.csv` with header `time_ms,IR,RED`
- Stores CSV on:
  - **SPIFFS** partition (`/spiffs/log.csv`)
  - A **flash-backed FAT** partition used for **USB Mass Storage (MSC)** (`/data/log.csv`)
...
- On the USB drive (when exposed): `log.csv`
- Also stored internally in SPIFFS: `/spiffs/log.csv`
